...
class CephCrushMap():
    ...
    def _max_pool_size_for_rule(self, rule_id):
        """Return the maximum replica size across all pools using a rule.

        This is needed to determine whether OSD count imbalance across
        failure domains creates unusable capacity.
        """
        if not self.ceph_report:
            return 0

        pools = self.ceph_report.get('osdmap', {}).get('pools', [])
        sizes = [p['size'] for p in pools if p.get('crush_rule') == rule_id]
    ...
    def _check_class_imbalance(self, buckets, fd_bucket_ids,
        ...
        for cls in sorted(all_classes):
            ...
            if len(nonzero) == 1 and len(counts) > 1:
                ...
                continue
            # Ignore trivial single-OSD differences in small buckets
            # (e.g. 2v1, 3v2, 4v3) that pass the ratio check but
            # are not operationally significant.
            elif max(nonzero) - min(nonzero) < 2:
                continue
    ...
    def crushmap_osd_count_imbalanced_buckets(self):
        ...
        imbalanced = []
        for rid, tree, failure_domain in self._get_in_use_trees_and_fdomains():
            fd_bucket_ids = self._collect_fd_buckets(
                buckets, tree, failure_domain)
            if len(fd_bucket_ids) < 2:
                continue

            # OSD count imbalance can create unusable capacity when
            # failure domains <= replica count. When there are more
            # failure domains than replicas, CRUSH places data
            # proportionally to weight and imbalance is less of an issue.
            # So skip such buckets (e.g., 5 racks/AZs and size=3).
            max_size = self._max_pool_size_for_rule(rid)
            if max_size and len(fd_bucket_ids) > max_size:
                continue
    ...
    def _collect_fd_buckets(self, buckets, start_bucket_id, failure_domain):
        ...
        Skips 0-weight buckets as they are excluded from placement
        consideration, consistent with _is_bucket_imbalanced().
        """
        result = []
        ...
        for item in bucket["items"]:
            if item["id"] not in buckets:
                continue
            # Skip 0-weight buckets as they are excluded from placement
            # consideration.
            if item["weight"] == 0:
                continue
